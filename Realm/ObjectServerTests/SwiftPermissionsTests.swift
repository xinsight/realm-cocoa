...
class SwiftPermissionsAPITests: SwiftSyncTestCase {
    ...
    func add(user: SyncUser, toRole roleName: String, inRealm realm: Realm) {
        let user = realm.create(RealmSwift.PermissionUser.self, value: [user.identity!], update: .all)
        realm.create(PermissionRole.self, value: [roleName], update: .all).users.append(user)
    }
